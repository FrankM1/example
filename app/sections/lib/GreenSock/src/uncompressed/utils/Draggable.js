...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push(function () {
	...
	_gsScope._gsDefine("utils.Draggable", ["events.EventDispatcher", "TweenLite", "plugins.CSSPlugin"], function (EventDispatcher, TweenLite, CSSPlugin) {
		...
		_tempTransforms = [],
		    ...
		    _getOffset2DMatrix = function _getOffset2DMatrix(e, offsetOrigin, parentOffsetOrigin, zeroOrigin) {
		...
		_isArrayLike = function _isArrayLike(e) {
		    ...
		    _isClickable = function _isClickable(_x) {
			var _again = true;
			...
			_function: while (_again) {
				...
				var data;
				if (!e || !e.getAttribute || e.nodeName === "BODY") {
					return false;
				} else {
					if ((data = e.getAttribute("data-clickable")) === "true" || data !== "false" && (e.onclick || _clickableTagExp.test(e.nodeName + "") || e.getAttribute("contentEditable") === "true")) {
						return true;
					} else {
						_x = e.parentNode;
						_again = true;
