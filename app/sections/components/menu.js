...
var libSpanify = require('../lib/spanify');
...
var Menu = (function () {
	function Menu() {
		...
		this.createBounds();
		this.isMobile = config.isMobile || config.isTablet;
	...
	_createClass(Menu, [{
		...
		value: function init() {
			...
			pleaseAjax.get(this.template, {
				success: function success(object) {
					...
					self.ui = queryDomComponents({ el: self.el });

					var spans = libSpanify({
						els: self.ui.span,
		...
		value: function createDOM() {
			...
			this.header = domCreateElement({
				selector: 'header',
				styles: 'js-header'
			});

			this.logo = domCreateElement({
				selector: 'a',
				link: '/cases',
				styles: 'logo',
		...
		value: function addEvents() {
			...
			domEvents.on(this.logo, 'click', function (e) {
				e.preventDefault();

				if (self3.isMobile) return;

				app.go(e.currentTarget.getAttribute('href'));
			});

			domEvents.on(this.bind, 'click', this.toggle);

			document.ontouchmove = function (event) {
				if (domClasses.has(config.$body, 'is-menu')) {
					event.preventDefault();
				}
			};

			utils.js.sliceArray(this.ui.el).forEach(function (el) {
				domEvents.on(el, 'click', self3.toggle);

				if (!self3.isMobile) {
					domEvents.on(el, 'mouseenter', self3.hoverIn);
					domEvents.on(el, 'mouseleave', self3.hoverOut);
				}
			});
		}
	}, {
		...
		value: function toggle(e) {
			...
			if (!this.isMobile && e.currentTarget.tagName == 'A') {
				...
				this.titleOut = domSelect('.title', e.currentTarget);
			} else {
				this.titleOut = null;
			}
			...
			if (this.open) {
				setTimeout(function () {
					return self4.animateOut();
				}, 300);
			} else {
				this.animateIn();
			}

			domClasses.toggle(this.root, 'is-menu');
		}
	}, {
		key: 'close',
		value: function close() {
			...
			domClasses.remove(this.root, 'is-menu');
		}
	}, {
		key: 'setState',
		...
		value: function setActive() {
			...
			var currentPanel = $('.menu-el[href="/' + currentPage + '"]');
			$('.menu-el').removeClass('is-active');
		...
		value: function hoverIn(e) {
			...
			var title = domSelect('.title', e.currentTarget);

			if (config.isMobile) {
				domCss(title, 'height', 'auto');
				return;
			}

			if (!domClasses.has(this.el, 'is-ready')) return;
		...
		value: function animateIn() {
			...
			domClasses.add(this.el, 'is-ready');
			...
			domEvents.on(this.logo, 'click', this.close);
		}
	}, {
		key: 'animateOut',
		value: function animateOut() {
			...
			domClasses.remove(this.el, 'is-ready');
			...
			domEvents.off(this.logo, 'click', this.close);
		}
	}]);
